...
//    }
    ...
    */
    ...
    //  Диапазоны   //
    /*
     Диапазон составляет набор значений или некоторый интервал. Для создания диапазона применяется оператор ".." :
    */

    //val range = 1..5 // [1, 2, 3, 4, 5]

    /*
     Этот оператор принимает два значения - границы диапазона, и все элементы между этими значениями
     (включая их самих) составляют диапазон.

     Диапазон не обязательно должен представлять числовые данные. Например, это могут быть строки:
    */

//    val range = "a".."b"

    /*
    Оператор ".." позволяет создать диапазон по нарастающей, где каждый следующий элемент будет больше
    предыдущего. С помощью специальной функции downTo можно построить диапазон в обратном порядке:
    */
...
//    val range2 = 5 downTo 1 // 5 4 3 2 1
    ...
    /*
    Еще одна специальная функция step позволяет задать шаг, на который будут изменяться элементы:
    */
...
//    val range2 = 10 downTo 1 step 3 //10 7 4 1
    ...
    /*
    Еще одна функция until позволяет не включать верхнюю границу диапазона
    */
...
//    val range2 = 1 until 9 step 2 // 1 3 5 7
    ...
    /*
    Для хранения наборов значений в Kotlin, как и в других языках программирования, можно использовать
    массивы. При этом массив может хранить данные только одного того же типа. в Kotlin массивы представлены
    типом Array

    При определении массива после типа Array в угловых скобках необходимо указать, объекты какого типа
    могут храниться в массиве. Например, определим массив целых чисел:
    */

//    val numbers : Array<Int>;

    /*
    С помощью функции arrayOf() можно передать набор значений, которые будут составлять массив:
    */
...
//    val numbers = arrayOf(1, 2, 3, 4, 5);
    ...
    Но что, если мы хотим создать массив, элементы которого будут не установлены
    (например, мы изначально не знаем, что это будут за элементы)? В этом случае для создания
    массива мы можем использовать встроенную функцию arrayOfNulls(). Она создает массив из
    элементов, каждый из которых имеет значение null - то есть фактически не установленное
    значение. В эту функцию передается количество элементов:
    */

//    val numbers = arrayOfNulls<Int>(3);

    /*
    После названия функции в угловых скобках указывается тип элементов массива. Так, в примере выше это
    тип Int. В данном случае создается массив из трех элементов, каждый из которых равен по умолчанию
    null, то есть не имеет никакого значения.

    Также имеется один способ сознать массив и инициализировать его значениями:
    */

//    val numbers = Array(3, {5});

    /*
    Здесь применяется конструктор класса Array. В этот конструктор передаются два значения. Первое
    значение - число 3 - указывает, сколько элементов будет в массиве. В данном случае 3 элемента.
    Второе значение - число 5 представляет выражение, которое генерирует элементы массива.
    Оно заключается в фигурные скобки. В данном случае в фигурных скобках стоит число 5,
    то есть все элементы массива будут представлять число 5. Таким образом массив будет
    состоять из трех пятерок.
...
//    val numbers = Array(3, {i++ * 2}); // [2, 4, 6]
    ...
    /*
    В данном случае элемент массива является результатом умножения переменной i на 2.
    При этом при каждом обращении к переменной i ее значение увеличится на единицу.

    С помощью индексов мы можем обратиться к определенному элементу в массиве.
    Индексация начинается с нуля, то есть первый элемент будет иметь индекс 0.
...
//    }
    ...
    /*
    В данном случае переменная numbers представляет массив чисел. При переборе этого массива в цикле каждый
    его элемент оказывается в переменной number, значение которой, к примеру, можно вывести на консоль.

    Как и в случае с последовательностью мы можем проверить наличие или отсутствие элементов в массиве
